...
public class HTTPReaderTest {
	...
	/**
	 * Test for ReadLineLFCR with a fake server closing the connection before the
	 * line is fully read We expect an HTTPException as the server close the
	 * connection before sending a complete LFCR terminated line.
	 * 
	 * @throws java.io.IOException
	 */
	@Test
	...
	public void testReadChunks() throws IOException {
		...
		try {
			...
			assertEquals("Wikipedia in\r\n\r\nchunks.",
					StandardCharsets.US_ASCII.decode(reader.readChunks().flip()).toString());
		} finally {
			server.shutdown();
		}
	}
