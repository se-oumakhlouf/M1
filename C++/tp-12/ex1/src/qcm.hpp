...
#define ANSWER_00 false // ordonné pas un ABR (arbre binaire de recherche)
...
#define ANSWER_01 \
    false // std::unordered_set<T> à besoin d'une fonction de hachage et d'une fonction d'égalité
          // on repère cela tout en haut de la doc `class Hash = std::hash<Key>` | `class Pred =
          // std::equal_to<Key>`

// Je peux définir et insérer une instance de MyObject dans un std::set<MyObject>.
#define ANSWER_02 true // basé sur un ABR, juste besoin d'un `operator<`, `class Compare = std::less<Key>`
...
#define ANSWER_10 true // efface un élément d'un std::vector via un itérateur rend invalide l'itérateur

// L'instruction `vec.erase(it_v2);` invalide it_v4.
#define ANSWER_11 \
    true // supprimer un élément d'un std::vector déplace les autres éléments vers la gauche pour combler le
         // trou donc supprimer it_v2 invalide it_v4

// L'instruction `vec.erase(it_v4);` invalide it_v2.
#define ANSWER_12 false // it_v2 est avant it_v4, donc it_2 n'est pas affecté

// L'instruction `vec.push_back(6);` invalide it_v4.
#define ANSWER_13 \
    true // cela provoque une réallocation (manque de capacité), tous les itérateurs existants sont invalidés
...
// Documentation :
// Addition, removal and moving the elements within the list or across several lists does not
// invalidate the iterators or references. An iterator is invalidated only when the corresponding element is
// deleted.
...
#define ANSWER_41 false // référence universelle
